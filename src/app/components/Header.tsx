...
export default function Header() {
  ...
  return (
    // Full-width overlay at the very top. This removes the page’s white band above the nav.
    <header className="fixed inset-x-0 top-0 z-50">
      {/* Ensure it hugs the very top on iOS (no notch gap) */}
      <div className="pt-[env(safe-area-inset-top,0px)]">
        {/* Your original bar, unchanged in size/margins, just using mt-4 instead of top-4 */}
        <div className="mx-auto w-[92%] sm:w-11/12 md:w-5/6 lg:w-3/4 mt-4 rounded-[40px] backdrop-blur-xl bg-white/20 border border-white/30 shadow-lg">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 py-2 flex items-center justify-between">
            ...
            <nav className="hidden md:flex items-center gap-12 text-[15px]">
              <a href="#page" className="hover:text-zinc-700 transition-colors" data-split>
                Home
              </a>
              <a href="./aboutus.tsx" className="hover:text-zinc-700 transition-colors" data-split>
                About Us
              </a>
            </nav>
            ...
            {/* Hamburger (mobile) — unchanged behavior */}
            <button
        ...
        {mobileOpen && (
          ...
          >
            <motion.nav
              ...
              exit={{ y: -24, opacity: 0 }}
              className="absolute left-1/2 top-[calc(env(safe-area-inset-top,0px)+16px)] w-[92%] -translate-x-1/2 rounded-2xl bg-white text-zinc-900 shadow-xl p-5"
            >
              ...
              <ul className="mt-4 space-y-3 text-base font-medium">
                <li>
                  <a href="#page" onClick={() => setMobileOpen(false)} className="block py-2">
                    Home
                  </a>
                </li>
                <li>
                  <a href="./aboutus.tsx" onClick={() => setMobileOpen(false)} className="block py-2">
                    About Us
                  </a>
                </li>
                <li className="pt-2">
